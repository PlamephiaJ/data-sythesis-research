...
def _to_optional_int(value) -> Optional[int]:
    ...
    try:
        ...
        raise ValueError(f"mask_cluster_id must be int or null, got: {value}")
...
def _build_masked_train_eval_dataset(
    cfg: DictConfig,
    ...
    if mask_cluster_id is None:
        logger.info("mask_cluster_id is null, using original train/validation split.")
        return base_train_ds, base_eval_ds

    if not masked_phish_data:
        raise ValueError(
    ...
    if "cluster_id" not in masked_all_ds.column_names:
        raise ValueError(
            f"masked phish data does not contain `cluster_id`: {masked_phish_data}"
        )
    if "phish" not in masked_all_ds.column_names:
        raise ValueError(
            f"masked phish data does not contain `phish`: {masked_phish_data}"
        )

    def _phish_to_int(ex):
        val = ex.get("phish")
        ex["phish"] = int(val) if val is not None else 0
        return ex

    masked_all_ds = masked_all_ds.map(_phish_to_int)
    masked_phish_ds = masked_all_ds.filter(
        lambda x: int(x.get("phish", 0)) == 1,
        num_proc=cfg.data.num_proc,
    )
    phish_split = masked_phish_ds.train_test_split(
        test_size=1 - train_split_ratio,
        seed=split_seed,
    )

    train_phish_ds = phish_split["train"]
    ...
    eval_benign_ds = base_eval_ds.filter(
        lambda x: int(x.get("phish", 0)) == 0,
        num_proc=cfg.data.num_proc,
    )
    ...
    train_ds = concatenate_datasets(
        [
            ...
            _with_required_columns(train_phish_ds),
        ]
    )
    eval_ds = concatenate_datasets(
        [_with_required_columns(eval_benign_ds), _with_required_columns(eval_phish_ds)]
    )
    ...
    logger.info(
        "Final dataset sizes after masking: train=%s, eval=%s",
        len(train_ds),
        len(eval_ds),
    )
...
def _run_training(cfg: DictConfig):
    ...
    try:
        logger.info("Starting masked training. run_dir=%s", run_dir)
        ...
        extra_train_dir = base_train._resolve_data_path(cfg.data.extra_train_dir)
        augment_data = base_train._resolve_data_path(cfg.data.augment_data)
        generated_eval_dir = base_train._resolve_data_path(cfg.data.generated_eval_dir)

        train_ds, eval_ds = _build_masked_train_eval_dataset(cfg, cache_dir)

        if extra_train_dir:
            extra_ds = base_train._load_json_data(extra_train_dir)
            train_ds = concatenate_datasets(
                [train_ds, _with_required_columns(extra_ds)]
            )
        ...
        summary = {
            ...
            "mask_cluster_id": mask_cluster_id,
            "masked_phish_data": getattr(
                cfg.data,
                "masked_phish_data",
                "data_phish/masked/phish_clustered_all.jsonl",
        ...
        if mask_cluster_id is not None:
            eval_masked_cluster_ds = eval_ds.filter(
                lambda x: _parse_cluster_id(x.get("cluster_id")) == mask_cluster_id,
                num_proc=cfg.data.num_proc,
            )
            if len(eval_masked_cluster_ds) > 0:
                eval_masked_cluster_tok = base_train._prepare_dataset(
                    eval_masked_cluster_ds,
                    tokenizer,
                    cleaner,
                    used_max_length,
                    cfg.data.num_proc,
                )
                if len(eval_masked_cluster_tok) > 0:
                    cluster_pred = trainer.predict(eval_masked_cluster_tok)
                    cluster_labels = np.array(eval_masked_cluster_tok["labels"])
                    cluster_metrics = base_train._compute_metrics_from_arrays(
                    ...
                    logger.info(
                        "Masked cluster eval metrics computed for cluster_id=%s with %s samples.",
                        mask_cluster_id,
                        len(eval_masked_cluster_tok),
                    )
                else:
                    summary["masked_cluster_eval"] = {
                        "cluster_id": mask_cluster_id,
        ...
        logger.info("Masked training finished successfully.")

    except Exception:
        logger.exception("Masked training crashed. See log file: %s", log_file)
        raise
